...
namespace TankRevival
{
    public sealed class Projectile : MonoBehaviour
    {
        ...
        internal bool IsPoolManaged { get; private set; }
        ...
        internal void MarkPoolManaged() => IsPoolManaged = true;
        ...
        {
            if (!IsPoolManaged && ProjectilePool.TryRouteFreshProjectile(this, direction, ownerTeam, damage, speed, color, ammo))
                return;
            InitializeFromPool(direction, ownerTeam, damage, speed, color, ammo);
        }

        internal void InitializeFromPool(Vector2 direction, Team ownerTeam, int damage, float speed, Color color, AmmoType ammo)
        {
            EnsureRuntimeParts();
            IsPoolManaged = true;
            _releasing = false;
            ...
            transform.rotation = Quaternion.Euler(0f, 0f, Mathf.Atan2(shotDirection.y, shotDirection.x) * Mathf.Rad2Deg - 90f);
        ...
        private void Update()
        {
            ...
            if (Time.time >= _dieAt) { Recycle(); return; }
            if (Ammo == AmmoType.Basic || Time.time < _nextTrail) return;

            float interval = Ammo == AmmoType.Plasma ? 0.025f : 0.045f;
            if (WarfarePerformanceGovernor.Tier == WarfarePerformanceGovernor.BudgetTier.Balanced) interval *= 1.45f;
            else if (WarfarePerformanceGovernor.Tier == WarfarePerformanceGovernor.BudgetTier.Survival) interval *= 2.2f;
        ...
        private bool CanPenetrate() => _penetrations > 0 && (Ammo == AmmoType.ArmorPiercing || Ammo == AmmoType.Plasma);

        private void ApplyStatus(Health target)
        {
            if (target == null || target.IsDead) return;
            if (Ammo != AmmoType.EMP && Ammo != AmmoType.Incendiary) return;
            var status = target.GetComponent<CombatStatus>();
            if (status == null) status = target.gameObject.AddComponent<CombatStatus>();
            if (Ammo == AmmoType.EMP) status.ApplyEmp(2.4f);
        ...
        private void Detonate(Health primary)
        {
            ...
            int hitCount = Physics2D.OverlapCircleNonAlloc(transform.position, radius, SplashBuffer);
            for (int i = 0; i < hitCount; i++)
            {
                Collider2D hit = SplashBuffer[i];
        ...
        private void OnDestroy() => ProjectilePool.Forget(this);
    }
}
