...
{
    /// <summary>
    /// v1.3 STEEL DOCTRINES — EAGLE FORTRESS
    /// Builds a destructible defense layer around Orzelek every round. The core remains fully
    /// destructible; these modules buy time, intercept shells and create tactical priorities.
    /// </summary>
    public sealed class EagleFortressDirector : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (_round != _game.CurrentRound || _eagle == null)
                TryBuildFortress();
        }
        ...
        private void TryBuildFortress()
        {
            ...
            if (_fortressRoot != null)
                Destroy(_fortressRoot.gameObject);
            ...
            int armorHp = Mathf.Clamp(2 + _round / 28, 2, 5);
            ...
            if (_round >= 35)
                CreateArmorNode("FORTRESS_CENTER", core + new Vector2(0f, 1.15f), new Vector2(0.82f, 0.52f), armorHp + 1);

            CreateSentry("EAGLE_SENTRY_ALPHA", core + new Vector2(-2.55f, 0.25f), _round);
            if (_round >= 18)
                CreateSentry("EAGLE_SENTRY_BRAVO", core + new Vector2(2.55f, 0.25f), _round);

            if (_round >= 25)
                CreateRepairRelay(core + new Vector2(0f, -0.52f), _round);
        ...
        {
            int hp = Mathf.Clamp(2 + round / 32, 2, 5);
            ...
            var sentry = go.AddComponent<EagleSentryModule>();
            sentry.Initialize(_game, round);
        }

        private void CreateRepairRelay(Vector2 position, int round)
        {
            int hp = Mathf.Clamp(2 + round / 35, 2, 4);
            ...
            var relay = go.AddComponent<EagleRepairRelay>();
            relay.Initialize(_eagle, round);
        }
        ...
        private int ActiveModules()
        {
            ...
            {
                Health module = _modules[i];
                if (module != null && !module.IsDead) count++;
            }
            return count;
        }
        ...
        private void OnGUI()
        {
            ...
            GUI.Label(new Rect(x + 12f, y + 51f, width - 24f, 18f), _round >= 25 ? "SENTRY NETWORK + REPAIR RELAY ACTIVE" : "SENTRY NETWORK ACTIVE", _body);

            if (Time.unscaledTime < _alertUntil)
                GUI.Label(new Rect(x + 12f, y + 68f, width - 24f, 18f), _alert, _warning);
        }
    }
    ...
    public sealed class EagleSentryModule : MonoBehaviour
    {
        ...
        private EnemyTank FindTarget()
        {
            ...
            EnemyTank best = null;
            float bestDistance = Mathf.Lerp(6.6f, 8.6f, (_round - 1f) / 99f);
    ...
    public sealed class EagleRepairRelay : MonoBehaviour
    {
        ...
        {
            _eagle = eagle;
            _charges = Mathf.Clamp(1 + round / 35, 1, 3);
            _nextRepair = Time.time + 12f;
        }

        private void Update()
        {
            if (_eagle == null || _eagle.IsDead || _charges <= 0 || Time.time < _nextRepair) return;
            _nextRepair = Time.time + 15f;
