#!/usr/bin/env python3
"""Deterministically stage and package one exact v12.9 Unity Windows candidate."""
...
ATTRIBUTION_NAME="V129_COMPONENT_WARFARE_BUILD.txt"
SHA_NAME="V129_COMPONENT_WARFARE_SHA256.txt"
PROVENANCE_NAME="V129_COMPONENT_WARFARE_PROVENANCE.json"
TREE_NAME="V129_COMPONENT_WARFARE_BUILD_TREE.txt"
DEFAULT_ZIP="TankRevivalOverdrive-v12.9-component-warfare-Windows-x64.zip"
...
def package_candidate(build_root: Path,output_dir: Path,candidate_sha: str,commit_sha: str,unity_version: str,version: str,zip_name: str=DEFAULT_ZIP,diagnostics_path: Path|None=None):
    ...
    provenance={"schema":"tank-revival-v12.9-component-warfare/v1","candidate_sha":candidate_sha,"commit_sha":commit_sha,"unity_version":unity_version,"version":version,"source_runtime":runtime.relative_to(build_root).as_posix(),"executable":EXE_NAME,"data_directory":DATA_NAME,"package":zip_name,"sha256":digest,"bytes":zp.stat().st_size,"file_count":len(files)}
...
def main(argv=None):
    ...
    try: result=package_candidate(a.build_root,a.output_dir,a.candidate_sha,a.commit_sha,a.unity_version,a.version_file.read_text(encoding="utf-8").strip(),a.zip_name,a.diagnostics_path)
        ...
        print(f"v12.9 packaging FAILED: {exc}",file=sys.stderr)
