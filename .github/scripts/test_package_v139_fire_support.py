...
MODULE = HERE / "package_v139_fire_support.py"
spec = importlib.util.spec_from_file_location("v139_pack", MODULE)
...
def make_runtime(root: Path) -> Path:
    ...
    (runtime / mod.EXE_NAME).write_bytes(b"MZ-v139-test")
...
def package_once(base: Path, name: str):
    ...
    out = base / (name + "-out")
    make_runtime(build)
    return mod.package_candidate(build, out, SHA, SHA, UNITY, "v13.9.0-dev"), out
...
def main() -> int:
    with tempfile.TemporaryDirectory() as td:
        ...
        assert p1["version"] == "v13.9.0-dev"
        assert p1["schema"] == "tank-revival-v13.9-fire-support/v1"
        ...
        with zipfile.ZipFile(z1) as z:
            ...
            assert "version=v13.9.0-dev" in attribution
        ...
        try:
            mod.package_candidate(mismatch_build, mismatch_out, SHA, "b" * 40, UNITY, "v13.9.0-dev")
        except mod.PackageError:
            pass
        else:
            raise AssertionError("candidate/commit mismatch unexpectedly succeeded")

    print("v13.9 deterministic packager tests: PASS")
    return 0
