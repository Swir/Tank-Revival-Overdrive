#!/usr/bin/env python3
"""Deterministically stage/package one exact v13.6 Battlefield Sensor Fusion Windows candidate."""
...
ATTRIBUTION_NAME="V136_SENSOR_FUSION_BUILD.txt"
SHA_NAME="V136_SENSOR_FUSION_SHA256.txt"
PROVENANCE_NAME="V136_SENSOR_FUSION_PROVENANCE.json"
TREE_NAME="V136_SENSOR_FUSION_BUILD_TREE.txt"
DEFAULT_ZIP="TankRevivalOverdrive-v13.6-sensor-fusion-Windows-x64.zip"
...
def package_candidate(build_root:Path,output_dir:Path,candidate_sha:str,commit_sha:str,unity_version:str,version:str,zip_name:str=DEFAULT_ZIP,diagnostics_path:Path|None=None):
    ...
    if version!="v13.6.0-dev": raise PackageError(f"unexpected version {version!r}")
    ...
    provenance={"schema":"tank-revival-v13.6-sensor-fusion/v1","candidate_sha":candidate_sha,"commit_sha":commit_sha,"unity_version":unity_version,"version":version,"source_runtime":runtime.relative_to(build_root).as_posix(),"executable":EXE_NAME,"data_directory":DATA_NAME,"package":zip_name,"sha256":digest,"bytes":zp.stat().st_size,"file_count":len(files)}
...
def main(argv=None):
    ...
    try: result=package_candidate(a.build_root,a.output_dir,a.candidate_sha,a.commit_sha,a.unity_version,a.version_file.read_text(encoding="utf-8").strip(),a.zip_name,a.diagnostics_path)
        ...
        print(f"v13.6 packaging FAILED: {exc}",file=sys.stderr)
