#!/usr/bin/env python3
"""Deterministically stage/package one exact v14.1 Counter-Observation Windows candidate."""
...
ATTRIBUTION_NAME = "V141_COUNTER_OBSERVATION_BUILD.txt"
SHA_NAME = "V141_COUNTER_OBSERVATION_SHA256.txt"
PROVENANCE_NAME = "V141_COUNTER_OBSERVATION_PROVENANCE.json"
TREE_NAME = "V141_COUNTER_OBSERVATION_BUILD_TREE.txt"
DEFAULT_ZIP = "TankRevivalOverdrive-v14.1-counter-observation-Windows-x64.zip"
...
def discover_runtime(root: Path) -> Path:
    ...
    if len(exes) != 1 or len(datas) != 1 or len(pairs) != 1:
        raise PackageError(
            f"expected exactly one EXE/Data pair; exes={len(exes)} data={len(datas)} pairs={len(pairs)}"
        )
...
def package_candidate(
    ...
    if version != "v14.1.0-dev":
        raise PackageError(f"unexpected version {version!r}")
    ...
    provenance = {
        "schema": "tank-revival-v14.1-counter-observation/v1",
        "candidate_sha": candidate_sha,
...
def main(argv=None):
    ...
    try:
        result = package_candidate(
            ...
            a.commit_sha,
            a.unity_version,
            a.version_file.read_text(encoding="utf-8").strip(),
            a.zip_name,
            a.diagnostics_path,
        )
    except (OSError, PackageError, ValueError) as exc:
        print(f"v14.1 packaging FAILED: {exc}", file=sys.stderr)
        try:
            print(describe_tree(a.build_root.resolve()), file=sys.stderr)
        except Exception:
            pass
        return 2
