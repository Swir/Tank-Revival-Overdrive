#!/usr/bin/env python3
"""Deterministically stage one exact v14.2 full-play Windows release candidate."""
...
ATTRIBUTION_NAME = "V142_FULL_PLAY_BUILD.txt"
...
def package_candidate(build_root: Path, output_dir: Path, candidate_sha: str, commit_sha: str,
    ...
    if not build_root.is_dir():
        raise PackageError(f"missing build root {build_root}")
    ...
    if commit_sha != candidate_sha:
        raise PackageError("commit SHA must equal candidate SHA")
    if version != "v14.2.0":
        raise PackageError(f"unexpected final release version {version!r}")
    ...
    for required in ("BUILD_INFO.txt", "DEMO_MANIFEST.txt", "README_DEMO.txt"):
        if not (stage / required).is_file():
            raise PackageError(f"release runtime missing {required}")
    ...
    (stage / "RELEASE_NOTES.txt").write_text(
        "TANK REVIVAL: ORZEL OVERDRIVE v14.2.0\n"
        "Full-play Windows x64 candidate. 100-round campaign, qualified v14.2 deception/EMCON warfare, "
    ...
    provenance = {
        "schema": "tank-revival-v14.2-full-play/v1",
        "candidate_sha": candidate_sha,
...
def main(argv=None):
    ...
    a = p.parse_args(argv)
    try:
        result = package_candidate(a.build_root, a.output_dir, a.candidate_sha, a.commit_sha,
                                   a.unity_version, a.version_file.read_text(encoding="utf-8").strip(),
                                   a.zip_name, a.diagnostics_path)
    except (OSError, PackageError, ValueError) as exc:
        print(f"v14.2 full-play packaging FAILED: {exc}", file=sys.stderr)
        return 2
