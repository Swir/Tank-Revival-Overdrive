...
spec=importlib.util.spec_from_file_location("package_v130_encounter_warfare",HERE/"package_v130_encounter_warfare.py")
assert spec and spec.loader
pkg=importlib.util.module_from_spec(spec); spec.loader.exec_module(pkg)
SHA="c"*40

class V130PackagerTests(unittest.TestCase):
    def make_player(self,root:Path,leaf="StandaloneWindows64"):
        runtime=root/leaf; runtime.mkdir(parents=True); (runtime/pkg.EXE_NAME).write_bytes(b"MZ-v130")
        ...
        return runtime
    def call(self,build:Path,out:Path): return pkg.package_candidate(build,out,SHA,SHA,"6000.3.17f1","v13.0.0-dev")
    def test_happy_path_and_provenance(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(info["candidate_sha"],SHA); self.assertFalse((source/pkg.ATTRIBUTION_NAME).exists()); self.assertTrue((out/pkg.PROVENANCE_NAME).is_file())
