...
spec=importlib.util.spec_from_file_location("package_v135_battlefield_weather",HERE/"package_v135_battlefield_weather.py")
assert spec and spec.loader
pkg=importlib.util.module_from_spec(spec); spec.loader.exec_module(pkg)
SHA="d"*40

class V135PackagerTests(unittest.TestCase):
    def make_player(self,root:Path,leaf="StandaloneWindows64"):
        runtime=root/leaf; runtime.mkdir(parents=True); (runtime/pkg.EXE_NAME).write_bytes(b"MZ-v135")
        ...
        return runtime
    def call(self,build:Path,out:Path): return pkg.package_candidate(build,out,SHA,SHA,"6000.3.17f1","v13.5.0-dev")
    def test_happy_path_and_provenance(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(info["candidate_sha"],SHA); self.assertEqual(info["version"],"v13.5.0-dev"); self.assertEqual(info["schema"],"tank-revival-v13.5-battlefield-weather/v1")
            self.assertFalse((source/pkg.ATTRIBUTION_NAME).exists()); self.assertTrue((out/pkg.PROVENANCE_NAME).is_file())
    ...
    def test_wrong_version_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            r=Path(td); self.make_player(r/"build")
            with self.assertRaises(pkg.PackageError): pkg.package_candidate(r/"build",r/"out",SHA,SHA,"6000.3.17f1","v13.4.0-dev")
    def test_mismatched_candidate_commit_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            r=Path(td); self.make_player(r/"build")
            with self.assertRaises(pkg.PackageError): pkg.package_candidate(r/"build",r/"out",SHA,"f"*40,"6000.3.17f1","v13.5.0-dev")
