...
MODULE = HERE / "package_v142_deception.py"
spec = importlib.util.spec_from_file_location("v142_pack", MODULE)
...
def make_runtime(root: Path) -> Path:
    ...
    (runtime / mod.EXE_NAME).write_bytes(b"MZ-v142-test")
...
def package_once(base: Path, name: str):
    ...
    out = base / (name + "-out")
    make_runtime(build)
    return mod.package_candidate(build, out, SHA, SHA, UNITY, "v14.2.0-dev"), out
...
def main() -> int:
    with tempfile.TemporaryDirectory() as td:
        ...
        assert p1["version"] == "v14.2.0-dev"
        assert p1["schema"] == "tank-revival-v14.2-deception/v1"
        ...
        with zipfile.ZipFile(z1) as z:
            ...
            assert "version=v14.2.0-dev" in attribution
        ...
        try:
            mod.package_candidate(mismatch_build, mismatch_out, SHA, "b" * 40, UNITY, "v14.2.0-dev")
        except mod.PackageError:
            pass
        else:
            raise AssertionError("candidate/commit mismatch unexpectedly succeeded")

        wrong_version_build = root / "wrong-version-build"
        wrong_version_out = root / "wrong-version-out"
        make_runtime(wrong_version_build)
        try:
            mod.package_candidate(wrong_version_build, wrong_version_out, SHA, SHA, UNITY, "v14.1.0-dev")
        except mod.PackageError:
            pass
        else:
            raise AssertionError("wrong v14.2 version unexpectedly accepted")

    print("v14.2 deterministic packager tests: PASS")
    return 0
