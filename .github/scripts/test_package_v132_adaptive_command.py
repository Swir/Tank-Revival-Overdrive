...
spec=importlib.util.spec_from_file_location("package_v132_adaptive_command",HERE/"package_v132_adaptive_command.py")
assert spec and spec.loader
pkg=importlib.util.module_from_spec(spec); spec.loader.exec_module(pkg)
SHA="e"*40

class V132PackagerTests(unittest.TestCase):
    def make_player(self,root:Path,leaf="StandaloneWindows64"):
        runtime=root/leaf; runtime.mkdir(parents=True); (runtime/pkg.EXE_NAME).write_bytes(b"MZ-v132")
        ...
        return runtime
    def call(self,build:Path,out:Path): return pkg.package_candidate(build,out,SHA,SHA,"6000.3.17f1","v13.2.0-dev")
    def test_happy_path_and_provenance(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(info["candidate_sha"],SHA); self.assertEqual(info["version"],"v13.2.0-dev"); self.assertFalse((source/pkg.ATTRIBUTION_NAME).exists()); self.assertTrue((out/pkg.PROVENANCE_NAME).is_file())
    ...
    def test_wrong_version_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            r=Path(td); self.make_player(r/"build")
            with self.assertRaises(pkg.PackageError): pkg.package_candidate(r/"build",r/"out",SHA,SHA,"6000.3.17f1","v13.1.0-dev")
