...
spec=importlib.util.spec_from_file_location("package_v134_tactical_terrain",HERE/"package_v134_tactical_terrain.py")
assert spec and spec.loader
pkg=importlib.util.module_from_spec(spec); spec.loader.exec_module(pkg)
SHA="e"*40

class V134PackagerTests(unittest.TestCase):
    def make_player(self,root:Path,leaf="StandaloneWindows64"):
        runtime=root/leaf; runtime.mkdir(parents=True); (runtime/pkg.EXE_NAME).write_bytes(b"MZ-v134")
        ...
        return runtime
    def call(self,build:Path,out:Path): return pkg.package_candidate(build,out,SHA,SHA,"6000.3.17f1","v13.4.0-dev")
    def test_happy_path_and_provenance(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(info["candidate_sha"],SHA); self.assertEqual(info["version"],"v13.4.0-dev"); self.assertEqual(info["schema"],"tank-revival-v13.4-tactical-terrain/v1")
    ...
    def test_wrong_version_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            r=Path(td); self.make_player(r/"build")
            with self.assertRaises(pkg.PackageError): pkg.package_candidate(r/"build",r/"out",SHA,SHA,"6000.3.17f1","v13.3.0-dev")
    def test_mismatched_candidate_commit_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            r=Path(td); self.make_player(r/"build")
            with self.assertRaises(pkg.PackageError): pkg.package_candidate(r/"build",r/"out",SHA,"f"*40,"6000.3.17f1","v13.4.0-dev")
